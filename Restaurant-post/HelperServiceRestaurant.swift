...
class HelperServiceRestaurant {
    ...
    static func uploadDataToServer(data: Data, name: String, desc: String, price: String, type: String, genre: String, cuisine: String, onSuccess: @escaping () -> Void) {
        // NSUUID
        ...
        storageRef.putData(data, metadata: nil) { (metadata, error) in
            ...
            storageRef.downloadURL { (url, error) in
                ...
                if let metaImageUrl = url?.absoluteString {
                    print(metaImageUrl)
                    self.sendDataToDatabase(photoUrl: metaImageUrl, name: name, desc: desc, price: price, type: type, genre: genre, cuisine: cuisine, onSuccess: onSuccess)
                }
            }
        }
    }
    
    static func sendDataToDatabase(photoUrl: String, name: String, desc: String, price: String, type: String, genre: String, cuisine: String, onSuccess: @escaping () -> Void) {
        ...
        newPostReference.setValue(["1) uid": currentUserId, "2) Photo Url": photoUrl, "3) Meal Name": name, "4) Meal Desc": desc, "5) Meal Price": price, "6) Meal Type": type, "7) Meal Genre": genre, "8) Meal Cuisine": cuisine], withCompletionBlock: { (error, ref) in
            if error != nil {
                print(error!.localizedDescription)
                return
            }
